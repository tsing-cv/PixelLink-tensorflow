...
def imshow(winname, img, block = True, position = None, maximized = False, rgb = False, use_opencv=False):
...
def get_contour_region_in_min_area_rect(img, cnt):
    ...
    new_corners = np.cast['int'](np.transpose(new_corners))
...
def fill_bbox(img, box, color = 1):
    ...
    """
    np.testing.assert_equal(np.shape(box), (4, 2))
    cnt = to_contours(box)

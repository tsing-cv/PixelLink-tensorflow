...
def get_neighbours(x, y, neighbour_type=cfgs.num_neighbours):
    if neighbour_type == 4:
        return get_neighbours_4(x, y)
    else:
        return get_neighbours_8(x, y)

# cpdef int is_valid_cord(int x, int y, int w, int h):
